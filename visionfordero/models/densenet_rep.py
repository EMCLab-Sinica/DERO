...
class RepConv(nn.Module):
    def __init__(self, in_planes, planes, kernel_size=3,
        ...
        self.groups = groups
        if self.groups == 1:
            self.conv11= nn.Conv2d(in_planes, planes, kernel_size=1,stride=stride, groups= groups, padding=0 )
            self.bnsc11   = nn.BatchNorm2d(planes)
            if self.stride == 1 and self.in_planes == self.planes:
                self.bnid   = nn.BatchNorm2d(planes)                         
    def forward(self, x):
     
        out = self.bn(self.conv(x))
        if self.groups == 1:
            out = out + self.bnsc11(self.conv11(x))
            if self.stride == 1 and self.in_planes == self.planes:
                out = out + self.bnid(x)
        return out
...
class _DenseLayer(nn.Module):
    def __init__(
        ...
        self.conv1 = RepConv(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False)

        self.norm2 = nn.BatchNorm2d(bn_size * growth_rate)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv2 = RepConv(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
...
class _DenseBlock(nn.ModuleDict):
    ...
    def forward(self, init_features: Tensor) -> Tensor:
        ...
        for name, layer in self.items():
            ...
            features = [F.pad(new_features, (0, 0, 0, 0, 0, self.num_input_features + i*self.growth_rate), "constant", 0)]
            i = i+1
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features: int, num_output_features: int) -> None:
        ...
        self.conv = RepConv(num_input_features, num_output_features, kernel_size=1, stride=1, bias=False)
        self.pool = nn.AvgPool2d(kernel_size=2, stride=2)


class DenseNet_rep(nn.Module):
...
def _densenet(
    ...
    **kwargs: Any,
    ...
    model = DenseNet_rep(growth_rate, block_config, num_init_features, **kwargs)
...
def densenet121_rep(*, weights: Optional[DenseNet121_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_rep:
...
def densenet161_rep(*, weights: Optional[DenseNet161_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_rep:
...
def densenet169_rep(*, weights: Optional[DenseNet169_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_rep:
...
def densenet201_rep(*, weights: Optional[DenseNet201_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_rep:

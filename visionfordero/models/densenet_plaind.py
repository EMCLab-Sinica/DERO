...
class DenseNet_plaind(nn.Module):
    ...
    def __init__(
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=growth_rate, num_output_features=num_features // 2)
                self.features.add_module("transition%d" % (i + 1), trans)
                num_features = num_features // 2
            else:
                trans = _Transition2(num_input_features=growth_rate, num_output_features=num_features)
                self.features.add_module("transition%d" % (i + 1), trans)
...
def _densenet(
    ...
    **kwargs: Any,
    ...
    model = DenseNet_plaind(growth_rate, block_config, num_init_features, **kwargs)
...
def densenet121_plaind(*, weights: Optional[DenseNet121_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_plaind:
...
def densenet161_plaind(*, weights: Optional[DenseNet161_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_plaind:
...
def densenet169_plaind(*, weights: Optional[DenseNet169_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_plaind:
...
def densenet201_plaind(*, weights: Optional[DenseNet201_Weights] = None, progress: bool = True, **kwargs: Any) -> DenseNet_plaind:
